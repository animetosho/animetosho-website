...
$comment = $AT->db->selectGetArray('feedback_comments', '`cid`='.$AT->input->cid, 'feedback_comments.*, users.username', array('joins' => array(array('left', 'users', 'uid'))));
if(!$comment)
...
if($AT->input->gettext && $AT->ajax_request) {
	...
	$form->start($AT->buildUrl('feedback'), $AT->postKey, 'editcomment', 'return AJS.x.submitEditComment(this,'.$comment['cid'].');', 'AJS.x.cancelEditComment();');
...
if($AT->user->isMod() && $comment['uid'] && stripos($AT->input->message, '[removed') !== false)
	$update['edit_lock'] = 1;
$AT->db->update('feedback_comments', $update, '`cid`='.$AT->input->cid);
...
if($AT->ajax_request)
{
	...
	$parser->deleteCached('toto_feedback_comment_'.$comment['cid']);
	echo $parser->parse('toto_feedback_comment_'.$comment['cid'], $AT->input->message);
}
else
{
